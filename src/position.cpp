...
Square Position::king(Color c) const { return lsb(pieces(c, King)); }
...
Bitboard Position::blockers(Color c) const { return state->blockers[c]; }
Bitboard Position::pinners(Color c) const { return state->pinners[c]; }
Bitboard Position::checkers() const { return state->checkers; }
